...
test('payment approval is explicit; changed facts require copy review before narration', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Commission creative direction', exact: true }).click()
  ...
  const voice = page.locator('.service-card').filter({ has: page.getByRole('heading', { name: 'Voiceover', exact: true }) })
  await voice.getByRole('button').click()
  await expect(page.getByRole('alert')).toContainText('Your brief changed')
  expect(purchases).toBe(1)
...
test('narrated video contains both a video track and an Opus audio track', async ({ page }) => {
  ...
  await page.goto('/')
  await expect(page.getByRole('button', { name: 'Export narrated promo', exact: true })).toBeVisible()
  const downloaded = page.waitForEvent('download')
  await page.getByRole('button', { name: 'Export narrated promo', exact: true }).click()
