...
import { X } from './Icons'
export function Modal({ title, children, onClose, className = '', actions }: { title: ReactNode; children: ReactNode; onClose: () => void; className?: string; actions?: ReactNode }) {
  ...
  return <dialog ref={ref} className={'modal ' + className} aria-labelledby={titleId} onCancel={onClose} onClick={event => { if (event.target === event.currentTarget) onClose() }}>
    <div className="modal-inner"><div className="flex items-center justify-between gap-4 mb-6">
      ...
      <div className="flex items-center gap-3">{actions}<button className="icon-button" aria-label="Close dialog" onClick={onClose}><X size={20} /></button></div>
    </div>{children}</div>
  </dialog>
}
