...
export function PurchaseDialog({ quote, onClose, onApprove, busy }: {
  quote: Quote; onClose: () => void; onApprove: (retryAcknowledged: boolean) => void; busy: boolean;
}) {
  ...
  return <Modal title="Review this purchase" onClose={onClose}>
    <p className="text-muted mb-6">{quote.retryOf ? 'This is a new paid attempt. The previous receipt stays in your campaign; an expired authorization does not prove it was never charged.' : 'One service for this campaign. Commission will sign only the exact payment shown below.'}</p>
    ...
    {quote.retryOf && <label className="flex items-start gap-3 mb-5 text-sm"><input type="checkbox" checked={retryAcknowledged} onChange={event => setRetryAcknowledged(event.target.checked)} disabled={busy} /><span>I checked the previous payment and understand this approves a new charge.</span></label>}
    <button className="button primary w-full" disabled={busy || !quote.ready || expired || Boolean(quote.retryOf && !retryAcknowledged)} onClick={() => onApprove(retryAcknowledged)}>{busy ? 'Authorizing…' : 'Approve ' + quote.amount + ' ' + quote.token + ' purchase'}<ArrowUpRight size={17} /></button>
